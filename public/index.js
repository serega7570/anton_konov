...
class Tasklist {
  ...
  onAddTaskButtonClick = async () => {
    const newTaskName = prompt('Введите название блюда:');
  ...
  onEditTask = async (taskID) => {
    ...
    const newTaskName = prompt('Введите новое название блюда', oldTaskName);
  ...
  onDeleteTaskButtonClick = async (taskID) => {
    ...
    if (!confirm(`Блюдо '${taskName}' будет удалено. Продолжить?`)) return;
  ...
  render() {
    ...
    button.innerHTML = 'Добавить блюдо';
